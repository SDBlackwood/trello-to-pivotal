...
class App {
    ...
    cardCommentItems(card){
        ...
        var ordered = orderComments(comments);
        return ordered;
    }
    //.data.text
    /**
    ...
    writePivotalTrackerRow(card) {
       ...
       // let type = this.cardType(card);
        ...
        let columnValues = [
            ...
            csvValueSafe(cardRequestedBy(card, this.actions)),
        ...
        for (let taskNum = 0; taskNum <= this.maxTasks(); taskNum++) {
            ...
            //console.log("List ", taskFromItem(checklistItems[taskNum]));
        };
        for (let commentNumber = 0; commentNumber <= this.maxComments(); commentNumber++) {
            //console.log("commentItems[commentNumber]", commentFromItem(commentItems[commentNumber]));
...
function cardRequestedBy(card, actions){

    let requester = '';
    for (let id in actions) {
       
        if (actions.hasOwnProperty(id)) {
            if (actions[id].type === 'createCard') {

                if (actions[id].data.card.id === card.id) {
                    requester = actions[id].memberCreator.fullName;
                    return requester;
                }
            }else if (actions[id].type === 'copyCard') {
...
function createComment(item){
    ...
    comment = comment + " (" + item.memberCreator.fullName;
    comment = comment +  " - Jun 11, 2019)";
    return comment;
}
/**
 Task from Trello checklist item;  empty string if input undefined; takes into account that a card can have multiple checklists, and so the max number of tasks per card is greater than for any one checklist
